...
},{"./gsap-core.js":"TNS6","./CSSPlugin.js":"bp4Z"}],"QvaY":[function(require,module,exports) {
"use strict";
...
var initTimeline1 = function initTimeline1() {
  console.log("init home");

  var tl = _gsap.default.timeline({}).from(document.querySelectorAll(".text-chunk"), {
    duration: 2,
    y: 50,
    alpha: 0,
    stagger: 0.2,
...
var initTimeline2 = function initTimeline2() {
  console.log("init projects");

  var tl = _gsap.default.timeline({}).to(document.querySelector(".slideshow-wrapper"), {
    duration: 2,
    y: 10,
    opacity: 1,
    ease: "power1.out"
      ...
      setTimeout(function () {
        var Slide = /*#__PURE__*/function () {
          function Slide(el) {
            ...
            this.DOM.imgWrap = this.DOM.el.querySelector(".slide__img-wrap"); // The revealer element (the element with the same color of the body that covers the image).

            this.DOM.revealer = this.DOM.imgWrap.querySelector(".slide__img-reveal"); // The title element.

            this.DOM.title = this.DOM.el.querySelector(".slide__title"); // The slide´s number element

            this.DOM.number = this.DOM.el.querySelector(".slide__number"); // The large preview elements (also an image and revealer for the cover/uncover effect)
          ...
          _createClass(Slide, [{
            ...
            value: function toggle(action, direction) {
              ...
              return new Promise(function (resolve, reject) {
                var revealerOpts = {
                  delay: action === "hide" ? 0 : _this.config.animation.duration / 2,
                ...
                if (direction === "left" || direction === "right") {
                  ...
                  titleOpts.ease = _this.config.animation.ease, titleOpts.startAt = action === "show" ? {
                    opacity: 1,
                    y: direction === "down" ? "-100%" : "100%"
                  } : {};
                ...
                TweenMax.to(_this.DOM.revealer, _this.config.animation.duration, revealerOpts); // Moving & fading the image (wrappper).

                TweenMax.to(_this.DOM.imgWrap, _this.config.animation.duration, imgOpts); // Moving & fading the title and number.

                TweenMax.to(_this.DOM.title, _this.config.animation.duration * 1.5, titleOpts);
                TweenMax.to(_this.DOM.number, _this.config.animation.duration, numberOpts);
              });
            ...
            value: function togglePreview(action) {
              ...
              return new Promise(function (resolve, reject) {
                // Hide/Show revealer.
                TweenMax.to(_this2.DOM.preview.revealer, _this2.config.animation.duration, {
                  delay: action === "hide" ? 0 : _this2.config.animation.duration / 2,
                  ease: _this2.config.animation.ease,
                  startAt: action === "hide" ? {
                    x: "0%",
                    y: "-100%"
                  } : {
                ...
                TweenMax.to(_this2.DOM.preview.imgWrap, _this2.config.animation.duration, {
                  delay: action === "hide" ? 0 : _this2.config.animation.duration / 2,
                  ease: _this2.config.animation.ease,
                  startAt: action === "hide" ? {} : {
                    opacity: 0,
                ...
                TweenMax.to([_this2.DOM.preview.title, _this2.DOM.preview.content], _this2.config.animation.duration, {
                  delay: action === "hide" ? 0 : _this2.config.animation.duration / 2,
                  ease: _this2.config.animation.ease,
                  startAt: action === "hide" ? {} : {
                    opacity: 0,
        ...
        var Slideshow = /*#__PURE__*/function () {
          function Slideshow(el) {
            ...
            this.DOM.previewCtrl = this.DOM.el.querySelector(".slidenav__preview"); // The slides.

            this.slides = [];
            Array.from(this.DOM.el.querySelectorAll(".slide")).forEach(function (slideEl) {
              return _this3.slides.push(new Slide(slideEl));
          ...
          _createClass(Slideshow, [{
            ...
            value: function togglePreview(action) {
              ...
              var processing = action === "enter" ? [this.slides[this.current].hide("up"), this.slides[this.current].showPreview()] : [this.slides[this.current].show("down"), this.slides[this.current].hidePreview()]; // Hide the next/prev controls.
            ...
            value: function toggleNavCtrls(action) {
              ...
              TweenMax.to([this.DOM.prevCtrl, this.DOM.nextCtrl], 0.5, {
                ...
                onStart: function onStart() {
                  return _this6.DOM.prevCtrl.style.pointerEvents = _this6.DOM.nextCtrl.style.pointerEvents = action === "enter" ? "none" : "auto";
                }
              });
            ...
            value: function navigate(direction) {
              ...
              Promise.all([this.slides[this.current].hide(direction), this.slides[nextSlidePos].show(direction)]).then(function () {
                // Update current.
                _this7.slides[_this7.current].setCurrent(false);

                _this7.current = nextSlidePos;
                _this7.isAnimating = false;

                _this7.slides[_this7.current].setCurrent();
              });
            }
          }]);
...
},{"@barba/core":"nGBO","gsap":"TpQl"}]},{},["QvaY"], null)